var listJSON=null;
var ID=null;
//FONCTION POUR LES ACTIONS XML les requêtes xml
function getXHR(){
    var xhrObject = null;
...
function parameters(address, parames){
    var pairs = document.URL.substring(address+1, document.URL.length).split('&');
    for (var i=0; i<pairs.length; i++) {
        nameVal = pairs[i].split('=');
        parames[nameVal[0]] = nameVal[1];
        console.log(parames[nameVal[0]]);
    }
    return parames
}
//vérification que l'ID est un nombre
function NombreSeulement(str) {
    return /^\d+$/.test(str);
}
...
function chargementPageProduit(json, id){
    ...
    if(id > ObjJSON.length || id < 1 || !NombreSeulement(id)){
        productPage[0].innerHTML = "<h1 style=\"color: black\">Page non trouvée!</h1>";
    } else {
        ID = id;
        for(var i = 0; i < ObjJSON.length; i++){
            if(ObjJSON[i].id == id){
                ...
                "</section>" +
                "<dialog id=\"dialog\" close> L'item <ins>" + ObjJSON[i].name + "</ins> est ajouté dans le panier</dialog>" +
                "<section>" +
                ...
                "<button class=\"btn\" title=\"Ajouter au panier\" type=\"submit\" onclick=\"submitForm(event);\">" +
...
function showShoppingCart(){
    ...
    for (var i = 0; i < window.localStorage.length; i++) {
        var key = window.localStorage.key(i);
        if (key != 'nbItems' && key != 'itemsInCart' && key != 'totalPrice' && !(userData(key)) && key != 'state'){
    ...
    function fillShoppingCart(value, index, array){
        ...
        '<td><div class=\"row\"><div class=\"col\"><button class = \"remove-quantity-button\" title=\"Retirer\" onclick=\"removeQuantity(event);\"'
        + disabled +'><i class=\"fa fa-minus\"></i></button></div><div class=\"col quantity\">' 
        + jsonItem.quantity
